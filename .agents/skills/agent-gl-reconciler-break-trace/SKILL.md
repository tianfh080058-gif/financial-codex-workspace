---
name: agent-gl-reconciler-break-trace
description: >-
  Root-cause a reconciliation break to its source transaction or posting — follow the audit
  trail from the break row back to the originating entry on each side and state what differs
  and why. Use after gl-recon has classified a break. Use when users need to trace
  reconciliation breaks, investigate ledger differences, and document root causes and
  resolving entries in general ledger reconciliation workflows.
---
